...
class Purge(commands.Cog):
    ...
    def check_should_keep(self, message, keep_media, keep_links):
        ...
        if keep_media:
            # Check attachments or embeds that look like media
    ...
    async def perform_scheduled_purge(self):
        ...
        for sch in schedules:
            ...
            def check(m):
                # Returns True if we should DELETE.
                # So we return False if we should KEEP.
                return not self.check_should_keep(m, keep_media, keep_links)

            try:
                # Purge entirely (limit=None)
    ...
    @app_commands.describe(
        ...
        amount="Number of messages or 'all'",
        scope_id="Optional: Channel ID, Category ID, or Server ID (defaults to current channel)"
    )
    async def purge_user(self, interaction: discord.Interaction, target: discord.User, amount: str, scope_id: str = None):
        ...
        if amount.lower() != "all":
            try:
                ...
                return await interaction.followup.send("❌ Amount must be a number or 'all'.")
            ...
            try:
                ...
                if s_id == interaction.guild.id:
                    ...
                    if isinstance(chan, discord.CategoryChannel):
                        ...
                        return await interaction.followup.send("❌ Invalid Scope ID. Must be Server, Category, or Text Channel ID.")
            except ValueError:
                return await interaction.followup.send("❌ Scope ID must be a number.")
        ...
        for channel in channels_to_purge:
            try:
                ...
                # If we have a limit, decrease it by what we just deleted (so we don't delete 100 per channel if request was 100 total)
                if limit is not None:
                    limit -= len(deleted)
                    if limit <= 0:
                        break
            except Exception as e:
                print(f"Failed to purge in {channel.name}: {e}")
    ...
    @app_commands.describe(
        ...
        keep_media="Keep images/videos?",
        keep_links="Keep messages with links?"
    )
    async def purge_messages(self, interaction: discord.Interaction, amount_or_till: str, message_id: str = None, keep_media: bool = False, keep_links: bool = False):
        ...
        if amount_or_till.lower() == "till":
            if not message_id:
                return await interaction.followup.send("❌ You must provide `message_id` when using 'till'.")
            try:
                # 'after' in purge means 'messages newer than this ID'
                # which effectively purges 'till' (down to) that message.
                after_msg = discord.Object(id=int(message_id))
            except ValueError:
                return await interaction.followup.send("❌ Invalid Message ID.")
        else:
            try:
                limit = int(amount_or_till)
            except ValueError:
                return await interaction.followup.send("❌ First argument must be a number or 'till'.")

        def check(m):
            # Return True to DELETE.
            # So return False if we want to KEEP.
            return not self.check_should_keep(m, keep_media, keep_links)
