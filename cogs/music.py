...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        # Initialize YTDL with default options
        if yt_dlp:
            yt_dlp.utils.bug_reports_message = lambda *args, **kwargs: ''
            opts = self.get_ytdl_opts() # Initial Load
            try:
    ...
    def get_ytdl_opts(self):
        ...
        opts = {
            ...
            # Post Processors: Convert everything to MP3 192kbps (High Quality)
            'postprocessors': [{
    ...
    async def play(self, interaction: discord.Interaction, query: str = None):
        ...
        # --- MODE 2: SEARCH / URL ---
            ...
            try:
                ...
                # Update options to capture cookies for search
                opts = self.get_ytdl_opts()
                with yt_dlp.YoutubeDL(opts) as ydl:
    ...
    async def checkmusic(self, interaction: discord.Interaction):
        ...
        # Check Cookies
        cookie_path = self.check_and_convert_cookies()
        cookie_status = f"✅ **Cookies Found:** `{cookie_path}`" if cookie_path else "❌ **No Cookies Found**"
