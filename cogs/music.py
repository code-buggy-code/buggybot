...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        # --- COOKIE CONVERSION (JSON -> Netscape) ---
        # Checks if cookies.json exists and cookies.txt doesn't, then converts it automatically!
        if not os.path.exists('cookies.txt') and os.path.exists('cookies.json'):
            print("🔄 Found cookies.json! Converting to Netscape format for yt-dlp...")
            try:
                ...
                with open('cookies.txt', 'w') as f:
                    ...
                    for c in cookies_json:
                        ...
                        domain = c.get('Host raw', '')
                        # Clean domain: remove protocol and trailing slash if present
                        domain = domain.replace('https://', '').replace('http://', '').rstrip('/')
                        
                        flag = "TRUE" if domain.startswith('.') else "FALSE"
                        path = c.get('Path raw', '/')
                        secure = "TRUE" if c.get('Send for raw', 'false').lower() == 'true' else "FALSE"
                        expiry = c.get('Expires raw', '0')
                        name = c.get('Name raw', '')
                ...
                print("✅ Successfully converted cookies.json to cookies.txt")
